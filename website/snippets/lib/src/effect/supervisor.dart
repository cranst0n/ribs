...
// supervisor-basic
IO<int> supervisorBasic() =>
    Supervisor.create().use(
...
IO<Unit> fireAndForget() =>
    Supervisor.create().use((supervisor) {
      return supervisor
          .supervise(IO.sleep(1.seconds).productR(() => IO.print('done')))
          .voided();
    });
// supervisor-fire-forget
...
IO<Unit> supervisorWaitForAll() =>
    IO.ref(false).flatMap((completed) {
      return Supervisor.create(waitForAll: true)
          .use((supervisor) {
            return supervisor
                .supervise(
                  IO.sleep(200.milliseconds)
                      .productR(() => completed.setValue(true)),
                )
                .voided();
          })
