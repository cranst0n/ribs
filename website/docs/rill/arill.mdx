...
import RillSnippet from '!!raw-loader!../../snippets/lib/src/rill/rill.dart';
...
emits `Chunk<O>` values and is stack-safe regardless of stream length. You
...
Effects within the stream are expressed with `evalMap` and `evalTap`, which
...
<CodeSnippet snippet={RillSnippet} section="rill-create" />
...
| `Rill.never` | A stream that never emits or terminates |

## Transforming a Rill

<CodeSnippet snippet={RillSnippet} section="rill-transform" />
...
| `concat` | Append one stream after another |
...
<CodeSnippet snippet={RillSnippet} section="rill-effects" />

`evalTap` is the stream equivalent of `flatTap` on `IO` — it executes a
...
`Rill.bracket(acquire, release)` ties a resource's lifetime to the stream's
lifetime. The release action runs **unconditionally** when the stream ends —
whether it completes normally, raises an error, or is canceled by the fiber
runtime.

<CodeSnippet snippet={RillSnippet} section="rill-resource" />
...
one of the interrupt combinators. When a stream is interrupted, all `bracket`
...
<CodeSnippet snippet={RillSnippet} section="rill-interrupt" />
...
expands into a **multi-element sub-stream** — use `parJoin`.
...
arrive in **completion order**: whichever inner stream emits next appears next,
regardless of its position in the outer stream.

### Fixed set of concurrent streams

<CodeSnippet snippet={RillSnippet} section="rill-par-join" />
...
`flatMap` + `parJoin` is the general pattern when the number of inner streams
is not known up front or each outer element should fan out into multiple values.

<CodeSnippet snippet={RillSnippet} section="rill-par-join-dynamic" />
...
`parJoin` when each element should produce a full sub-stream.
:::

### Error propagation and cancellation

If any inner stream raises an error, `parJoin` immediately cancels all
remaining inner streams and propagates the error to the outer stream. If the
outer stream itself is interrupted (e.g. via `interruptAfter`), all running
...
<CodeSnippet snippet={RillSnippet} section="rill-realworld" />
...
`parEvalMap` for `parEvalMapUnordered`. To run a background stream concurrently
