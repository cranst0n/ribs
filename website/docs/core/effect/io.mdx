...
import IOSnippet from '!!raw-loader!../../../snippets/lib/src/core/effect/io.dart';
...
<CodeSnippet language="dart" title="IO.async_" snippet={IOSnippet} section="error-handling-1" />
...
* **unsafeRunToFuture**: Evaluates the `IO` and returns a `Future` that will
complete with the value of the `IO` or any error that is encountered during
the evaluation.

* **unsafeRunToFutureOutcome**: Returns a `Future` that will complete with
