...
Let's work through what each of those properties means in practice:
...
* Mutating a variable
    ...
    <<< @/../snippets/lib/src/overview.dart#overview-1
* Printing to console

    <<< @/../snippets/lib/src/overview.dart#overview-2
* Changing a field of an object

    <<< @/../snippets/lib/src/overview.dart#overview-3
* Throwing an `Exception`

    <<< @/../snippets/lib/src/overview.dart#overview-4

Eliminating side-effects from our programs allows us to write pure functions.

## Pure Functions

A pure function should take its parameters, compute a result and return that
...
<<< @/../snippets/lib/src/overview.dart#overview-5

This simple function takes its inputs and the result of the function is
...
<<< @/../snippets/lib/src/overview.dart#overview-6

Now let's try substituting every occurrence of `b` with the expression that `b`
evaluated to, namely `a.add()`:

<<< @/../snippets/lib/src/overview.dart#overview-7
