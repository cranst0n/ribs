...
class ByteVector {
  ...
  ByteVector get reverse => ByteVector.fromList(_underlying.reversed.toList());
  ...
  Uint8List get toByteArray => _underlying;

  ByteVector update(int n, Byte byte) =>
      take(n).append(byte).concat(drop(n + 1));

  String get toHexString => isEmpty
      ? 'ByteVector.empty'
      : _underlying.toIList
          .map((b) => b.toRadixString(16).padLeft(2, '0'))
          .mkString(sep: '');

  String get toBinString => isEmpty
      ? 'ByteVector.empty'
      : _underlying.toIList
