...
final genMulticast4 = genIpv4.map((ip) {
  ...
  return Ipv4Address.fromInt(
    i,
  ).asMulticast().getOrElse(() => throw Exception('genMulticast4 failed: $ip'));
});

final genHostname = _genHostnameImpl();
final genIDN = _genIdnImpl();

final genMacAddress = Gen.listOfN(6, Gen.byte).map(
  (bytes) => MacAddress.fromByteList(
    bytes,
  ).getOrElse(() => throw Exception('genMacAddress failed: $bytes')),
);

final genPort = Gen.chooseInt(
  Port.MinValue,
  Port.MaxValue,
...
Gen<Cidr<A>> genCidr<A extends IpAddress>(Gen<A> genIp) => genIp.flatMap(
  (ip) =>
...
Gen<Hostname> _genHostnameImpl() {
  final genLabel = Gen.alphaNumChar.flatMap((first) {
    return Gen.chooseInt(0, 61).flatMap((middleLen) {
      return Gen.ilistOfN(
        middleLen,
        Gen.oneOfGen([Gen.alphaNumChar, Gen.constant('-')]),
      ).map((a) => a.mkString()).flatMap((middle) {
...
Gen<IDN> _genIdnImpl() {
  ...
  final genLabel = genChar.flatMap((first) {
    return Gen.chooseInt(0, 61).flatMap((middleLen) {
      return Gen.ilistOfN(
        middleLen,
        Gen.oneOfGen([genChar, Gen.constant('-')]),
      ).map((a) => a.mkString()).flatMap((middle) {
        return (middleLen > 0 ? Gen.some(Gen.alphaNumChar) : Gen.option(Gen.alphaNumChar))
            .map((s) => s.getOrElse(() => ""))
            .map((last) => '$first$middle$last')
