...
sealed class Resource<A> with Functor<A>, Applicative<A>, Monad<A> {
  ...
  static Resource<A> apply<A>(IO<(A, IO<Unit>)> resource) => applyCase(resource.map((tup) => tup(
  ...
  Resource<Either<RuntimeException, A>> attempt() => switch (this) {
        ...
        final _Bind<dynamic, A> b =>
          Resource.unit.flatMap((_) => b.source.attempt()).flatMap((att) => att.fold(
          ...
          return IO.bracketFull(
            ...
            (tuple) {
              ...
              return switch (stack) {
                ...
                final _StackFrame<dynamic, dynamic> f => loop(f.head(a), f.tail),
              };
            },
            (a, oc) {
              final (_, release) = a;
