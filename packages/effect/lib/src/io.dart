...
sealed class IO<A> with Functor<A>, Applicative<A>, Monad<A> {
  ...
  static IO<A> fromFutureF<A>(Function0<Future<A>> futF) => fromFuture(IO.delay(futF));
  ...
  /// evaluation.
  static IO<Unit> printErrLn(String message) => _platformImpl.printErrLn(message);
  ...
  static IO<A> raiseError<A>(Object error) => _Error(error);
  ...
  /// region.
  static IO<A> uncancelable<A>(Function1<Poll, IO<A>> body) => _Uncancelable(body);
  ...
  @override
  IO<B> ap<B>(covariant IO<Function1<A, B>> f) => flatMap((a) => f.map((f) => f(a)));
  ...
  /// value.
  IO<Either<Object, A>> attempt() => _Attempt(this);

  IO<A> attemptTap<B>(Function1<Either<Object, A>, IO<B>> f) => attempt().flatTap(f).rethrowError();
  ...
  IO<A> flatTap<B>(covariant Function1<A, IO<B>> f) => flatMap((a) => f(a).as(a));
  ...
  /// evaluating this [IO].
  IO<A> guarantee(IO<Unit> fin) => IO.uncancelable(
    (poll) => poll(
      this,
    ).onCancel(fin).onError((e) => fin.productR(() => IO.raiseError(e))).flatTap((_) => fin),
  );

  /// Executes the provided finalizer [fin] which can decide what action to
  /// take depending on the [Outcome] of this [IO].
  IO<A> guaranteeCase(Function1<Outcome<A>, IO<Unit>> fin) => IO.uncancelable(
    (poll) => poll(this)
        .onCancel(fin(Outcome.canceled()))
        .onError((err) => fin(Outcome.errored(err)))
        .flatTap((a) => fin(Outcome.succeeded(a))),
  );

  /// Intercepts any upstream Exception, returning the value generated by [f].
  IO<A> handleError(Function1<Object, A> f) => handleErrorWith((e) => IO.pure(f(e)));

  /// Intercepts any upstream Exception, sequencing in the [IO] generated by [f].
  IO<A> handleErrorWith(covariant Function1<Object, IO<A>> f) => _HandleErrorWith(this, Fn1(f));
  ...
  IO<A> iterateWhile(Function1<A, bool> p) => flatMap((a) => p(a) ? iterateWhile(p) : IO.pure(a));
  ...
  IO<A> onError(covariant Function1<Object, IO<Unit>> f) =>
      handleErrorWith((e) => f(e).attempt().productR(() => IO.raiseError<A>(e)));
  ...
  IO<Unit> parReplicate_(int n) => n <= 0 ? IO.unit : IO.both(this, parReplicate_(n - 1)).voided();
  ...
  /// Runs this [IO] [n] times, accumulating the result from each evaluation
  /// into an [IList].
  IO<IList<A>> replicate(int n) =>
      n <= 0 ? IO.pure(nil()) : flatMap((a) => replicate(n - 1).map((l) => l.prepended(a)));
      ...
      (a) => a.fold(
        ...
        (bWon) {
          ...
          return f.cancel().start().productR(
            () => IO.raiseError<A>(TimeoutException(duration.toString())),
          );
        },
      ),
    ),
  ...
  /// Note that [cond] is evaluated *after* evaluating this [IO] for each
  /// repetition.
  IO<Unit> untilM_(IO<bool> cond) => productR(() => cond.ifM(() => IO.unit, () => untilM_(cond)));
  ...
  void unsafeRunAndForget({
    IORuntime? runtime,
    int autoCedeN = IOFiber.DefaultAutoCedeN,
  }) => _unsafeRunFiber(() {}, (a) {}, (a) {}, runtime: runtime, autoCedeN: autoCedeN);
  ...
  Future<Outcome<A>> unsafeRunFutureOutcome({
    ...
    unsafeRunAsync(completer.complete, runtime: runtime, autoCedeN: autoCedeN);
  ...
  Future<A> unsafeRunFuture({
    IORuntime? runtime,
    int autoCedeN = IOFiber.DefaultAutoCedeN,
  }) => unsafeRunFutureCancelable(runtime: runtime, autoCedeN: autoCedeN).$1;
  ...
  Function0<Future<Unit>> unsafeRunCancelable({
    IORuntime? runtime,
    int autoCedeN = IOFiber.DefaultAutoCedeN,
  }) => unsafeRunFutureCancelable(runtime: runtime, autoCedeN: autoCedeN).$2;

  IOFiber<A> _unsafeRunFiber(
    Function0<void> canceled,
    Function1<Object, void> failure,
...
final class _Start<A> extends IO<IOFiber<A>> {
  ...
  IOFiber<A> createFiber(IORuntime runtime) =>
      IOFiber(ioa, runtime: runtime, autoCedeN: _autoCedeN);
...
final class _RacePair<A, B> extends IO<RacePairOutcome<A, B>> {
  ...
  IOFiber<A> createFiberA(IORuntime runtime, int autoCedeN) =>
      IOFiber(ioa, runtime: runtime, autoCedeN: autoCedeN);
  IOFiber<B> createFiberB(IORuntime runtime, int autoCedeN) =>
      IOFiber(iob, runtime: runtime, autoCedeN: autoCedeN);
...
final class IOFiber<A> {
  ...
  void _asyncContinueSuccessfulR() => _runLoop(_succeeded(_objectState.pop(), 0), _autoCedeN);

  void _asyncContinueFailedR() => _runLoop(_failed(_objectState.pop() as Object, 0), _autoCedeN);
  ...
  void _asyncContinueCanceledWithFinalizerR() {
    final cb = _objectState.pop() as Fn1<Either<Object, Unit>, void>;
      ...
      if (nextCede <= 0) {
        ...
        if (cur0 is _Pure) {
          ...
          if (ioa is _Pure) {
            ...
            cur0 =
                error == null ? _succeeded(cur0.right(result), 0) : _succeeded(cur0.left(error), 0);
          } else {
            final attempt = cur0;
          ...
          _runtime.scheduleAfter(cur0.duration, _resume);
          break;
        } else if (cur0 is _Now) {
  ...
  IO<dynamic> _failed(Object error, int depth) {
    ...
    switch (cont) {
      ...
      case _Cont.Attempt:
        _objectState.pop();
        return _succeeded(Left<Object, Never>(error), depth);
    }
  }
  ...
  IO<dynamic> _cancelationLoopSuccessK() {
    if (_finalizers.nonEmpty) {
      ...
      if (_objectState.nonEmpty) {
        ...
        cb.call(Right<Object, Unit>(Unit()));
      }
