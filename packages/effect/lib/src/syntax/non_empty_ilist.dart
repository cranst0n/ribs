...
extension IONonEmptyIListOps<A> on NonEmptyIList<A> {
  ...
  IO<IList<B>> flatTraverseIO<B>(Function1<A, IO<IList<B>>> f) => toIList().flatTraverseIO(f);
  ...
  /// that result is returned and any additional elements will not be evaluated.
  IO<IList<B>> traverseFilterIO<B>(Function1<A, IO<Option<B>>> f) => toIList().traverseFilterIO(f);
  ...
  /// possible.
  IO<Unit> parTraverseIO_<B>(Function1<A, IO<B>> f) => toIList().parTraverseIO_(f);
}

/// Operations available when [NonEmptyIList] elements are of type [IO].
