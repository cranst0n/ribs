...
final class PlatformImpl extends PlatformBase {
  @override
  IO<A> isolate<A>(IO<A> io, {String? debugName}) => IO.fromFutureF(
      () => Isolate.run(() => io.unsafeRunFuture(), debugName: debugName));
  ...
  IO<Unit> _printTo(Stdout s, String message) =>
      _opAndFlush(s, (std) => std.write(message));

  IO<Unit> _printlnTo(Stdout s, String message) =>
      _opAndFlush(s, (std) => std.writeln(message));
