...
final class TupleIOOpsGenerator {
  static String generate(int arity) {
    ...
    for (int tupleSize = 2; tupleSize <= arity; tupleSize++) {
      generatedCode.writeln(_generateTupleIOOpsExtension(tupleSize));
    }
  ...
  static String _generateTupleIOOpsExtension(int size) {
    final extensionName = 'Tuple${size}IOOps<${_typeParams(size).join(', ')}>';
    final onType = '(${_typeParams(size).map((t) => 'IO<$t>').join(',')})';
    ...
    final extension = Extension(
      (b) =>
          b
            ..name = extensionName
            ..docs.add(
              '/// Provides additional functions on a tuple of $size [IO]s.',
            )
            ..on = (refer(onType))
            ..methods.addAll([
              mapN(size),
  ...
  static Method tupledN(int size) {
    ...
    final returnType = 'IO<(${returnTypeParams.join(',')})>';
  ...
  static Method parTupledN(int size) {
    ...
    final impl =
        size == 2
            ? const Code('IO.both(\$1, \$2)')
            : const Code('IO.both(init.parTupled, last).map((t) => t.\$1.appended(t.\$2))');
