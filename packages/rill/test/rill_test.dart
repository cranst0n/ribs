...
void main() {
  ...
  test('exists', () async {
    ...
    final exists = await a.compile().toIList().unsafeRunToFuture();
    final notExists = await b.compile().toIList().unsafeRunToFuture();
  ...
  test('find', () async {
    ...
    final found = await r.compile().toIList().unsafeRunToFuture();

    final r2 = Rill.range(0, 100).find((a) => a == -1);
    final notFound = await r2.compile().toIList().unsafeRunToFuture();
  ...
  test('range', () async {
    ...
    final result = await r.compile().count().unsafeRunToFuture();
