...
class Scope {
  ...
  int get level {
    int go(Scope scope, int acc) => scope.parent.fold(() => acc, (a) => go(a, acc + 1));

    return go(this, 0);
  }

  IO<bool> _register(ScopedResource resource) => state.modify(
    (s) => s.fold(
  ...
  IO<Either<RuntimeException, Scope>> open(bool interruptible) {
    IO<Scope> createScope() {
      ...
      return this.interruptible.fold(
        () {
          final optFCtx = Option.when(
            () => interruptible,
            () => InterruptContext.create(newScopeId, IO.unit),
          );
          return optFCtx.sequence().flatMap((iCtx) => Scope.create(newScopeId, Some(this), iCtx));
        },
        (parentICtx) {
          return parentICtx
              .childContext(interruptible, newScopeId)
              .flatMap((iCtx) => Scope.create(newScopeId, Some(this), Some(iCtx)));
        },
      );
    }
      ...
      ScopedResource.create().flatMap((resource) {
        return IO.uncancelable((poll) {
          return acquire(poll).redeemWith(
            ...
            (r) {
              IO<Unit> finalizer(ExitCase ec) => IO.uncancelable((_) => release(r, ec));

              return resource.acquired(finalizer).flatMap((result) {
                if (result.exists(identity)) {
                  return _register(resource).flatMap(
                    (successful) =>
                        successful
                            ? IO.pure(Right(r))
                            : finalizer(
                              ExitCase.canceled(),
                            ).as(Left(RuntimeException('Acquire after scope closed'))),
                  );
                } else {
                  return finalizer(ExitCase.canceled()).as(
                    Left(
                      result.swap().getOrElse(() => RuntimeException('Acquire after scope closed')),
                    ),
                  );
                }
              });
  ...
  IO<IList<ScopedResource>> _resources() => state.value().map(
    (s) => s.fold(
      (s) => s.resources,
      () => IList.empty(),
    ),
  ...
  IO<Either<RuntimeException, Unit>> _traverseError<A>(
    ...
    Function1<A, IO<Either<RuntimeException, Unit>>> f,
      ...
      .traverseIO(f)
      .map(
        (results) => CompositeError.fromIList(
          results.foldLeft(
            IList.empty<RuntimeException>(),
            (acc, elem) => elem.fold((err) => acc.appended(err), (_) => acc),
          ),
        ).toLeft(() => Unit()),
      );
      ...
      state.modify((s) => (_ScopeState.closed, s)).flatMap((s) {
        return s.fold(
          (previous) => (
            ...
            (resultChildren, resultResources, _, _) =>
  ...
  IO<Scope> _openScope() => state.value().flatMap(
    (s) => s.fold(
      (_) => IO.pure(this),
      () => openAncestor(),
    ),
  ...
  IO<Scope> openAncestor() => parent.fold(
    () => IO.pure(this),
    (parent) => parent.state.value().flatMap(
      (parentState) => parentState.fold((_) => IO.pure(parent), () => parent.openAncestor()),
    ),
  );
  ...
  bool descendsFrom(UniqueToken scopeId) => _findSelfOrAncestor(scopeId).isDefined;
  ...
  IO<Option<Scope>> findInLineage(UniqueToken scopeId) =>
      IO.pure(_findSelfOrAncestor(scopeId)).orElse(() => _findSelfOrChild(scopeId));

  IO<Option<Scope>> _findSelfOrChild(UniqueToken scopeId) {
    IO<Option<Scope>> go(IList<Scope> scopes) {
      return scopes.uncons(
        (x) => x.fold(
          () => IO.pure(none()),
          (x) {
            final (scope, tail) = x;
            ...
            if (scope.id == scopeId) {
              ...
              return scope.state.value().flatMap(
                (s) => s.fold(
                  (s) {
                    if (s.children.isEmpty) {
                      return go(tail);
                    } else {
                      return go(s.children).flatMap(
                        (scope) => scope.fold(
                          () => go(tail),
  ...
  IO<Scope> shiftScope(UniqueToken scopeId, String context) => findStepScope(scopeId).flatMap(
    (s) => s.fold(
      () => IO.raiseError(
        RuntimeException("Scope lookup failed! Scope ID: $scopeId, Step: $context"),
      ),
      (scope) => IO.pure(scope),
    ),
  );
  ...
  IO<Option<Scope>> findStepScope(UniqueToken scopeId) {
    Scope go(Scope scope) => scope.parent.fold(() => scope, (parent) => go(parent));
    ...
    if (scopeId == id) {
      ...
      return parent.fold(
        ...
        (parent) => parent
            ...
            .flatMap(
              (s) =>
                  s.fold(() => go(this)._findSelfOrChild(scopeId), (scope) => IO.pure(Some(scope))),
            ),
      );
    }
  }

  IO<IOFiber<Unit>> interruptWhen(IO<Either<RuntimeException, Unit>> haltWhen) =>
      interruptible.fold(
        () => IO.pure(IOFiber(IO.unit)),
        (iCtx) {
          final outcome = haltWhen.map(
            (a) => a.fold(
  ...
  IO<Option<InterruptionOutcome>> get isInterrupted => _openScope().flatMap(
    (scope) => scope.interruptible.fold(
      () => IO.pure(none()),
      (iCtx) => iCtx.ref.value(),
    ),
  );

  IO<Either<InterruptionOutcome, A>> interruptibleEval<A>(IO<A> f) => _openScope()
      .map((a) => a.interruptible)
      .flatMap(
        (iCtx) => iCtx.fold(
          () => f.attempt().map((a) => a.leftMap((err) => Outcome.errored(err))),
          (iCtx) => iCtx.eval(f),
        ),
      );
  ...
  IO<Lease> lease() {
    return state.value().flatMap((s) {
      final children = s.fold(
        ...
        () => IO.raiseError<IList<Scope>>(RuntimeException('Scope closed at time of lease')),
      );
...
final class _ScopeOpen extends _ScopeState {
  ...
  _ScopeState unregisterChild(UniqueToken id) => children
      .deleteFirst((a) => a.id == id)
      .fold(
        () => this,
  ...
  @override
  B fold<B>(Function1<_ScopeOpen, B> ifOpen, Function0<B> ifClosed) => ifOpen(this);
...
final class _ScopeClosed extends _ScopeState {
  @override
  B fold<B>(Function1<_ScopeOpen, B> ifOpen, Function0<B> ifClosed) => ifClosed();
}
