...
abstract class KeyDecoder<A> {
  static KeyDecoder<A> instance<A>(Function1<String, Option<A>> decodeF) =>
      KeyDecoderF(decodeF);
  ...
  KeyDecoder<B> map<B>(Function1<A, B> f) =>
      KeyDecoder.instance((k) => decode(k).map(f));

  static KeyDecoder<String> string = lift(Some.new);

  static KeyDecoder<A> lift<A>(Function1<String, Option<A>> f) =>
      KeyDecoderF(f);
}
