...
Gen<Json> genNumberInt =
    Gen.integer.map((x) => JNumber(x)).map((number) => number.value.isFinite ? number : JNull());

Gen<Json> genNumberDouble = Gen.chooseDouble(-double.maxFinite, double.maxFinite,
        specials: ilist([double.infinity, double.negativeInfinity, double.nan]))
    .map((x) => JNumber(x))
    .map((number) => number.value.isFinite ? number : JNull());

Gen<JString> genString =
    Gen.chooseInt(1, 6).flatMap((n) => Gen.alphaNumString(n).map((x) => JString(x)));

Gen<JArray> genArray(int depth) =>
    Gen.listOf(Gen.chooseInt(0, _maxArraySize), genJsonAtDepth(depth - 1))
