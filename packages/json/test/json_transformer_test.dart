...
void main() {
  ...
  test('JsonTransformer.unwrapArray (bytes)', () async {
    ...
    ).openRead().transform(JsonTransformer.bytes(AsyncParserMode.unwrapArray));
  ...
  test('JsonTransformer.unwrapArray (string)', () async {
    final stream = File(
      'test_resources/streaming/large-array.json',
    ).openRead().map(utf8.decode).transform(JsonTransformer.strings(AsyncParserMode.unwrapArray));
  ...
  test('JsonTransformer.valueStream (bytes)', () async {
    final stream = File(
      'test_resources/streaming/line-delimited.json',
    ).openRead().transform(JsonTransformer.bytes(AsyncParserMode.valueStream));
  ...
  test('JsonTransformer.valueStream (strings)', () async {
    final stream = File(
      'test_resources/streaming/line-delimited.json',
    ).openRead().map(utf8.decode).transform(JsonTransformer.strings(AsyncParserMode.valueStream));
