...
void main(List<String> args) {
  ...
  ns.forEach((n) {
    ...
    final benchmarks = [
      // put
      ComparativeBenchmark('Dart Map', 'put', () => genDartMap(n),
          (m) => m..putIfAbsent(n, () => n.toString()), emitter),
      ComparativeBenchmark('FIC IMap', 'put', () => genFicIMap(n),
          (m) => m.putIfAbsent(n, () => n.toString()), emitter),
      ComparativeBenchmark(
          'Ribs IMap', 'put', () => genRibsIMap(n), (m) => m.updated(n, n.toString()), emitter),
      ComparativeBenchmark(
          'Ribs MMap', 'put', () => genRibsMMap(n), (m) => m..update(n, n.toString()), emitter),
      ComparativeBenchmark(
          'Dartz IMap', 'put', () => genDartzIMap(n), (m) => m.put(n, n.toString()), emitter),
      ComparativeBenchmark('Built Map', 'put', () => genBuiltMap(n),
          (m) => m.rebuild((p0) => p0.putIfAbsent(n, () => n.toString())), emitter),
      ComparativeBenchmark('Kt Map', 'put', () => genKtMap(n),
          (m) => m.plus(KtHashMap.from({n: n.toString()})), emitter),
      ...
      ComparativeBenchmark(
          'Dartz IMap', 'get', () => genDartzIMap(n), (m) => m..get(n ~/ 2), emitter),
      ...
      // remove (present)
      ComparativeBenchmark(
          'Dart Map', 'remove (present)', () => genDartMap(n), (m) => m..remove(n ~/ 2), emitter),
      ComparativeBenchmark(
          'FIC IMap', 'remove (present)', () => genFicIMap(n), (m) => m.remove(n ~/ 2), emitter),
      ComparativeBenchmark(
          'Ribs IMap', 'remove (present)', () => genRibsIMap(n), (m) => m.removed(n ~/ 2), emitter),
      ComparativeBenchmark(
          'Ribs MMap', 'remove (present)', () => genRibsMMap(n), (m) => m..remove(n ~/ 2), emitter),
      ComparativeBenchmark('Dartz IMap', 'remove (present)', () => genDartzIMap(n),
          (m) => m.remove(n ~/ 2), emitter),
      ComparativeBenchmark('Built Map', 'remove (present)', () => genBuiltMap(n),
          (m) => m.rebuild((p0) => p0.remove(n ~/ 2)), emitter),
      ComparativeBenchmark(
          'Kt Map', 'remove (present)', () => genKtMap(n), (m) => m.minus(n ~/ 2), emitter),

      // remove (absent)
      ComparativeBenchmark(
          'Dart Map', 'remove (absent)', () => genDartMap(n), (m) => m..remove(n), emitter),
      ComparativeBenchmark(
          'FIC IMap', 'remove (absent)', () => genFicIMap(n), (m) => m.remove(n), emitter),
      ComparativeBenchmark(
          'Ribs IMap', 'remove (absent)', () => genRibsIMap(n), (m) => m.removed(n), emitter),
      ComparativeBenchmark(
          'Ribs MMap', 'remove (absent)', () => genRibsMMap(n), (m) => m..remove(n), emitter),
      ComparativeBenchmark(
          'Dartz IMap', 'remove (absent)', () => genDartzIMap(n), (m) => m.remove(n), emitter),
      ComparativeBenchmark('Built Map', 'remove (absent)', () => genBuiltMap(n),
          (m) => m.rebuild((p0) => p0.remove(n)), emitter),
      ComparativeBenchmark(
          'Kt Map', 'remove (absent)', () => genKtMap(n), (m) => m.minus(n), emitter),
    ];
