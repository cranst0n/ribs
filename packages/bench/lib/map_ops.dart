...
void main(List<String> args) {
  ...
  Map<int, String> genDartMap(int n) => Map.fromEntries(
      List.generate(n, tabulateFn).map((e) => MapEntry(e.$1, e.$2)));
  ...
  built_list.BuiltMap<int, String> genBuiltMap(int n) =>
      built_list.BuiltMap.of(genDartMap(n));
  ...
  ns.forEach((n) {
    ...
    final benchmarks = [
      ...
      // get
      ComparativeBenchmark(
          'Dart Map', 'get', () => genDartMap(n), (m) => m..[n ~/ 2], emitter),
      ComparativeBenchmark(
      ...
      ComparativeBenchmark('Built Map', 'get', () => genBuiltMap(n),
          (m) => m..[n ~/ 2], emitter),
      ComparativeBenchmark(
          'Kt Map', 'get', () => genKtMap(n), (m) => m..get(n ~/ 2), emitter),

      // remove (present)
      ComparativeBenchmark('Dart Map', 'remove (present)', () => genDartMap(n),
          (m) => m..remove(n ~/ 2), emitter),
      ComparativeBenchmark('FIC IMap', 'remove (present)', () => genFicIMap(n),
          (m) => m.remove(n ~/ 2), emitter),
      ...
      ComparativeBenchmark(
          ...
          emitter),
      ComparativeBenchmark('Kt Map', 'remove (present)', () => genKtMap(n),
          (m) => m.minus(n ~/ 2), emitter),

      // remove (absent)
      ComparativeBenchmark('Dart Map', 'remove (absent)', () => genDartMap(n),
          (m) => m..remove(n), emitter),
      ComparativeBenchmark('FIC IMap', 'remove (absent)', () => genFicIMap(n),
          (m) => m.remove(n), emitter),
      ...
      ComparativeBenchmark('Kt Map', 'remove (absent)', () => genKtMap(n),
          (m) => m.minus(n), emitter),
    ];
