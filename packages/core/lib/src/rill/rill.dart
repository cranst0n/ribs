...
class Rill<O> implements Monad<O> {
  ...
  static Rill<Unit> fixedRate_(
    ...
    if (period.inMicroseconds == 0) {
      ...
      return Rill.eval(IO.sync(() => DateTime.now())).flatMap((now) {
  ...
  static Rill<O> fromStream<O>(Stream<O> s) => Rill.bracket(
        IO.sync(() => StreamQueue(s)),
        (q) =>
            IO.fromFuture<dynamic>(() => q.cancel() ?? Future.value()).voided,
      ).flatMap((q) => repeatEval(IO.fromFuture(() => q.next)));
  ...
  Rill<O2> then<O2>(Rill<O2> then) => flatMap((_) => then);
  ...
  Rill<O> get changes => filterWithPrevious((o1, o2) => o1 != o2);
  ...
  Rill<O> get scope => _pull.scope.rill;

  Rill<IList<O>> split(Function1<O, bool> f) {
    Pull<IList<O>, Unit> go(IList<O> buffer, Pull<O, Unit> r) {
      return r.uncons.flatMap((step) {
        return step.fold(
          (r) {
  ...
  Stream<O> get toStream {
    ...
    IO
        ...
        .bracket(
          (ctrl) => attempt()
              .evalTap((o) => IO.sync(() => o.fold(
                    (err) => ctrl.addError(err.$1, err.$2),
                    (o) => ctrl.add(o),
                  )))
              .compile
              .drain,
          (ctrl) => IO.sync(() => ctrl.close()).voided,
        )
        .unsafeRunAndForget();
  ...
  Rill<Unit> get voided => as(Unit());

  Rill<Tuple2<O, int>> get zipWithIndex => zip(Rill.iterate(0, (x) => x + 1));
...
extension RillNestedOps<O> on Rill<Rill<O>> {
  Rill<O> get flatten => flatMap(id);
}

extension RillOptionOps<O> on Rill<Option<O>> {
  Rill<O> get unNone => _pull.unNone.rill;
}
...
class RillCompiled<O> {
  ...
  IO<int> get count => _pull.fold(0, (x, __) => x + 1).map((t) => t.$2);
...
abstract class Pull<O, R> {
  ...
  Pull<O, Pull<O, R>> takeWhile(Function1<O, bool> p) => uncons.flatMap((step) {
        return step.fold(
          (r) => _Result(_Result(r)),
          (tuple) => tuple((hd, tl) {
            if (p(hd)) {
  ...
  Pull<Never, Either<R, Tuple2<O, Pull<O, R>>>> get uncons => _Uncons(this);
...
extension PullOps<O> on Pull<O, Unit> {
  ...
  Pull<O, Unit> get scope => _OpenScope(this, none());
}

extension PullNestedOps<O, R> on Pull<O, Pull<O, R>> {
  Pull<O, R> get flatten => flatMap(id);
}

extension PullOptionOps<O, R> on Pull<Option<O>, R> {
  Pull<O, R> get unNone => uncons.flatMap(
        (step) => step.fold(
          (r) => _Result(r),
          (tuple) => tuple((hd, tl) {
            return hd
...
abstract class _StepResult<O, R> {
  ...
  Either<R, Tuple2<O, Pull<O, R>>> get toUnconsResult => fold(
