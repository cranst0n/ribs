...
abstract class Either<A, B> implements Monad<B>, Foldable<B> {
  ...
  Either<B, A> get swap => fold((a) => right<B, A>(a), (b) => left(b));

  IList<B> get toIList => fold((_) => nil<B>(), (b) => IList.pure(b));

  Option<B> get toOption => fold((_) => none<B>(), Some.new);

  Validated<A, B> get toValidated => fold((a) => a.invalid(), (b) => b.valid());
...
extension EitherNestedOps<A, B> on Either<A, Either<A, B>> {
  Either<A, B> get flatten => fold((a) => Either.left<A, B>(a), id);
}
