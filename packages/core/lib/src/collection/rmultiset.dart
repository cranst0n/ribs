...
mixin RMultiSet<A> on RIterableOnce<A>, RIterable<A> {
  ...
  static RMultiSet<A> fromOccurences<A>(RIterableOnce<(A, int)> elems) =>
      switch (elems) {
        final RMultiSet<A> ms => ms,
  ...
  @override
  RMultiSet<B> collect<B>(Function1<A, Option<B>> f) =>
      RMultiSet.from(super.collect(f));

  RMultiSet<B> collectOccurances<B>(Function1<(A, int), Option<(B, int)>> f) =>
      flatMapOccurences(
        (kvs) => f(kvs).fold(
          () => const views.Empty(),
  ...
  @override
  RMultiSet<A> concat(covariant RIterableOnce<A> suffix) =>
      RMultiSet.from(super.concat(suffix));

  RMultiSet<A> concatOccurences(RIterable<(A, int)> that) =>
      RMultiSet.fromOccurences(that);
  ...
  @override
  RMultiSet<A> dropWhile(Function1<A, bool> p) =>
      RMultiSet.from(super.dropWhile(p));

  @override
  RMultiSet<A> filter(Function1<A, bool> p) => RMultiSet.from(super.filter(p));

  @override
  RMultiSet<A> filterNot(Function1<A, bool> p) =>
      RMultiSet.from(super.filterNot(p));
  ...
  @override
  IMap<K, RMultiSet<A>> groupBy<K>(Function1<A, K> f) =>
      super.groupBy(f).mapValues(RMultiSet.from);
  ...
  @override
  RIterator<RMultiSet<A>> grouped(int size) =>
      super.grouped(size).map(RMultiSet.from);
  ...
  RIterator<A> get iterator =>
      occurrences.iterator.flatMap((kv) => views.Fill(kv.$2, kv.$1));

  @override
  RMultiSet<B> map<B>(covariant Function1<A, B> f) =>
      RMultiSet.from(super.map(f));
  ...
  @override
  RMultiSet<B> scan<B>(B z, Function2<B, A, B> op) =>
      RMultiSet.from(super.scan(z, op));

  @override
  RMultiSet<B> scanLeft<B>(B z, Function2<B, A, B> op) =>
      RMultiSet.from(super.scanLeft(z, op));

  @override
  RMultiSet<B> scanRight<B>(B z, Function2<A, B, B> op) =>
      RMultiSet.from(super.scanRight(z, op));

  @override
  RMultiSet<A> slice(int from, int until) =>
      RMultiSet.from(super.slice(from, until));
  ...
  @override
  RMultiSet<A> takeWhile(Function1<A, bool> p) =>
      RMultiSet.from(super.takeWhile(p));
  ...
  @override
  RMultiSet<(A, B)> zip<B>(RIterableOnce<B> that) =>
      RMultiSet.from(super.zip(that));
  ...
  @override
  int get hashCode =>
      MurmurHash3.unorderedHash(occurrences, 'MultiSet'.hashCode);

  @override
  bool operator ==(Object other) =>
      identical(this, other) ||
      switch (other) {
        final RMultiSet<A> that => size == that.size &&
            occurrences.forall((kv) => that.get(kv.$1) == kv.$2),
        _ => false,
      };
}
